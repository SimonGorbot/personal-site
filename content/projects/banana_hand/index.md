...
The result is a 16 degree-of-freedom, 6 degree-of-actuation humanoid hand with five fingers, integrated force sensing, custom electronics, Rust firmware, and a ROS 2 software stack for teleoperation and grasp selection. More importantly, the full project was built around reproducibility: the CAD, PCB files, firmware, software, BOM, and assembly resources are open-source so that future teams can build on the platform instead of starting from scratch.
...
Robotic manipulation research has a hardware problem. When we looked into the space, we found that existing options tend to fall into two extremes. On one end, there are relatively cheap humanoid hands, but they often lack sufficient degrees of freedom, degrees of actuation, or sensing capabilities to support meaningful research or real-world tasks. On the other end, there are highly capable open-source and commercial hands, but they are extremely expensive and often out of reach for smaller labs, startups, and hobbyists.
...
BananaHand is split into three main sub-systems: mechanical design, high-level software, and the embedded system (hardware + firmware). My focus throughout the project was primarily on the embedded system.
![BananaHand vision based teleop UI demo](bh-demo-combined.gif "BananaHand vision based teleop UI demo.")

{{< expandable-h title="Mechanical" level="3" defaultOpen="false" >}}

Mechanically, the hand uses a five-finger anthropomorphic layout. Each of the four fingers is driven by a compact linear actuator through a coupled linkage mechanism. The base joint is actively driven, while the other finger joints move through the linkage geometry. This keeps the number of actuators low while still producing finger motion that resembles natural flexion.

{{< columns >}}
![Full hand CAD model](final_hand_design.png)
{{< column >}}
![Finger linkage mechanism.](finger-linkage-images.png)
{{< endcolumns >}}

The thumb uses a modified version of the same linkage concept, with an additional actuator for opposition. That thumb opposition axis is important because it lets the hand move beyond simple wrapping grasps and perform more useful interactions like pinch and tripod grasps.

The mechanical structure was designed to be manufacturable with accessible tools. Most custom components are FDM 3D printed, and the design uses off-the-shelf fasteners, bushings, pins, and actuators. Protective shells cover the palm, back of hand, and fingers, both to protect internal components and to provide more realistic contact surfaces for grasping.

{{< /expandable-h >}}

{{< expandable-h title="Electrical" level="3" defaultOpen="false" >}}

Electrically, the hand is built around a custom palm-mounted PCB. The board takes a single 12 V input, generates 3.3 V logic power locally, drives the actuators, reads actuator position feedback, reads motor current, collects force sensor data, and exposes communication interfaces for integration with external systems.

The board uses a dual-MCU architecture. An STM32G474 acts as the primary controller, handling upstream communication, motor control, actuator position sensing, current sensing, and telemetry. An STM32C071 acts as a dedicated sensing controller for the force-sensitive resistors. That split kept the force sensing path simple and isolated while leaving the more timing-sensitive control and communication work on the higher-performance G4.
...
On the **software** side, the project includes two main user-facing paths that we developed primarily as demonstrations of the hardware platform’s usability.

The first is vision-based teleoperation. A webcam tracks a human operator’s hand, extracts hand landmarks, normalizes the finger motion, and maps that motion into actuator commands for BananaHand. This provided a lightweight way to control the physical hand without requiring a glove, motion-capture system, or specialized hardware, and served as a clear demonstration that the hand could respond intuitively to real-time input.

The second is a scan-based grasp selection pipeline. Using RGB-D data from a RealSense camera, the software reconstructs a partial point cloud of an object, removes the table plane, extracts geometric descriptors, and classifies the object into one of the supported grasp families. This pipeline was intended to show that the hand could be integrated into a perception-driven workflow and execute different grasp strategies based on object geometry.

{{< /expandable-h >}}

{{< expandable-h title="Firmware" level="3" defaultOpen="false" >}}

The **firmware** sits between those high-level software commands and the physical hand. Commands arrive from the host, sensor readings are collected from the board, control loops compute motor outputs, and telemetry is sent back for visualization and debugging.

{{< /expandable-h >}}

## A Deeper Look At My Contributions

My main ownership areas were the electrical subsystem and embedded firmware. I was the only team member working directly on the electrical design, so I was responsible for turning the hand’s system requirements into PCB architecture that could fit inside the mechanical envelope and support the firmware/software goals.

That meant the electrical design could not be done in isolation. The PCB had to match the actuator choices from the mechanical team, support the sensing and telemetry requirements from the software/control side, fit inside the palm packaging, and remain simple enough to bring up within a 6-month timeline.

I also wrote a significant portion of the firmware, especially the lower-level embedded infrastructure needed to drive the actuators reliably from Rust.

### Electrical

The electrical subsystem is centred on one custom main board integrated into the palm of the hand. Its job is to consolidate power distribution, motor driving, position sensing, force sensing, and communication into a single compact PCBA.

The board takes in 12 V power, which is used directly by the PQ12P linear actuators. An onboard buck converter generates 3.3 V for the microcontrollers and logic-level components.

For motor control, the board includes eight DRV8876 H-bridge motor driver channels. The current build uses six actuator channels, but the extra channels gave us flexibility during design and integration. Each motor driver is controlled through PWM inputs from the STM32G474, allowing the firmware to command forward drive, reverse drive, braking, or coasting. The DRV8876 current sense outputs are routed back to ADC inputs on the G4, giving the firmware visibility into motor load and enabling future work on current-based protection or force estimation.

The Actuonix PQ12P actuators include built-in position feedback, so their position outputs are also routed to ADC channels on the G4. This let us close the loop around actuator position without adding external encoders, which helped reduce cost, part count, wiring, and mechanical complexity.

For tactile sensing, the board uses force-sensitive resistors (FSRs) connected through simple voltage divider circuits. These sensors are read by the STM32C071, which functions as a dedicated force-sensing front end. The C0 samples the FSR channels and streams readings to the G4 over UART. This kept the force acquisition path modular and reduced the amount of sensor-handling load on the primary controller.

The board also exposes UART, CAN, and half-duplex RS485. UART was used for development and system testing, while CAN and RS485 remain available for future integration with robot arms or more robust distributed systems.

A major part of the electrical work was not just schematic capture and layout, but making the design practical to build and debug. Before committing to the  PCB, I validated core functions with development boards and breakout hardware. After that, I designed the integrated board in KiCad and had it assembled through JLCPCB.

Bring-up followed a sub-circuit and functionality-based approach, where each unique function of the PCBA was validated independently before full system integration and installation into the hand.

The final board met the MVP requirements: it powered the system, drove the actuators, read force and position signals, communicated with the host software, and fit into the hand.

### Firmware

On the firmware side, the hand runs Rust firmware using Embassy. The final architecture uses two firmware targets: one for the STM32G474 primary controller and one for the STM32C071 sensing controller.

The STM32C071 firmware is responsible for force sensing. It samples the FSR channels, packages the readings, and sends them to the G4. The STM32G474 firmware handles the rest of the real-time behavior: receiving commands from the host, receiving force data from the C0, sampling actuator positions and motor currents, running the control loop, generating PWM outputs, and sending telemetry back to the PC.

The G4 firmware is structured around concurrent Embassy tasks. Separate tasks handle host command reception, telemetry transmission, force packet reception from the C0, position ADC sampling, and current ADC sampling. Shared state is used so the control loop can read coherent snapshots of the latest commands and sensor values without blocking the rest of the system.

The main control loop runs at 200 Hz. It supports position control and force control modes. In position mode, the controller drives actuators toward commanded positions using feedback from the PQ12P potentiometers. In force mode, the controller uses FSR data from the C0 path to regulate contact force where force feedback is available. The output of the control loop is converted into motor commands and then into PWM outputs for the motor drivers.

One of my most important firmware contributions was building a custom low-level HAL for the STM32G474 HRTIM peripheral. The G4’s high-resolution timer was the right hardware block for generating many synchronized PWM channels, but the support we needed was not exposed through Embassy HAL at the time. To solve that, I wrote a   HAL layer that exposed HRTIM subtimers as usable PWM channels in a way that fit into the Embassy-based firmware architecture.

That HAL handled HRTIM setup, subtimer configuration, PWM period calculation, GPIO alternate-function setup, and activation of the configured timer channels. In the final firmware, those HRTIM channels are mapped directly to actuator driver inputs for the hand’s motor channels.

This was one of my favourite parts of the project because. It sits right at the boundary between hardware capability and software abstraction. The STM32G474 had the peripheral features we needed, but making them easily usable in the rest of the codebase required building the missing layer myself.

In the end, the embedded stack became the bridge between the high-level ROS system and the physical hardware. The software could publish desired hand states, the firmware could translate those into real actuator behaviour, and the returned telemetry gave us enough observability to tune, debug, and demonstrate the system.

## Closing Thoughts

- very proud of this project
- learned a lot
- made my first contribution to open source
- had a lot of fun at symposium
- inspire youth
